...
function render() {
  ...
  const filtered = products.filter((product) => current === "All" || product.type === current);
...
window.setStoreCategory = (category) => {
  current = ["All", "Businesses", "Vehicles", "MLOs"].includes(category) ? category : "All";
  render();
};
...
function renderCart() {
  const selected = cart.map((id) => products.find((product) => String(product.id) === String(id))).filter(Boolean);
  cartCount.textContent = selected.length;
  if (!selected.length) {
    cartItems.innerHTML = `<p class="emptyCart">Your cart is empty.</p>`;
  } else {
    cartItems.innerHTML = selected.map((product) => `
      <div class="cartItem">
    ...
    cartItems.querySelectorAll(".remove").forEach((button) => {
      button.addEventListener("click", () => { remove(button.dataset.id); renderCart(); });
    });
  }
  cartTotal.textContent = money(selected.reduce((total, product) => total + (Number(product.price) || 0), 0));
}
...
if (exploreBtn) exploreBtn.addEventListener("click", (event) => {
